class DataViz:
  ...
  def __bivariatePlot(self, group_data:list,
    ...
    if group_data[0] and group_data[1]:
      # Stacked countplot
      df_plot = input_table.groupby(variables).size().reset_index().pivot(columns=variable[0], index=variable[1], values=0)
      output_graph = df_plot.plot(kind='bar', stacked=True)

    elif not (group_data[0] or group_data[1]):
      # Scatter plot
      output_graph = sns.scatterplot(data=input_table, x=variable[0], y=variable[1])
    else:
      # Boxplot
      if group_data[0]:
  ...
  def createGraph(self, group_data:list, file_path:str, variables:list) -> object:
    ...
    if len(variables)==2:
      output_graph = self.__bivariatePlot(group_data, file_path, variables)
    elif len(variables)==1:
      output_graph = self.__univariatePlot(group_data[0], file_path, variables[0])
    else:
      raise Exception('Error on createGraph method:   Input variables must be a list of strings with length 1 or 2')
